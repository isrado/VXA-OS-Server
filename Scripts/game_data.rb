  ...
  def load_game_data
	...
	def load_enemies
		...
		(1...enemies.size).each do |enemy_id|
			...
			respawn_time = Note.read_number('RespawnTime', enemies[enemy_id].note)
			$data_enemies[enemy_id].respawn_time = respawn_time > 0 ? respawn_time : RESPAWN_TIME
	...
	def load_skills
		...
		(1...skills.size).each do |skill_id|
			...
			$data_skills[skill_id].area = Note.read_number('Area', skills[skill_id].note)
		end
	end
	...
	def load_items
		...
		(1...items.size).each do |item_id|
			...
			$data_items[item_id].area = Note.read_number('Area', items[item_id].note)
