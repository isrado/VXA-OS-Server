...
# ** Game_General
#------------------------------------------------------------------------------
#  Este script lida com funções especiais do servidor.
#------------------------------------------------------------------------------
#  Autor: Valentine
#==============================================================================
...
module Game_General
  ...
  def illegal_name?(name)
	...
	def teleport_player(player, name, map_id, x, y)
		@clients.each do |client|
			...
			if name == 'all' && client.in_game?
				...
				alert_message(client, Constants::ALERT_TELEPORTED)
	...
	def give_gold(player, name, amount)
		...
		@log.add(player.group, :blue, "#{player.user} deu #{amount} moedas de ouro para #{name}.")
	end
	...
	def ban(client, type, name, days)
		...
		@log.add(client.group, :blue, "#{client.user} baniu #{name} por #{days} dias.")
	end
	...
	def admin_message(player, message)
		@clients.each { |client| send_admin_command(client, Constants::COMMAND_MSG, message) if client&.in_game? }
