...
module Game_Guild
  ...
  def create_guild(client, name, flag)
		if @guilds.has_key?(name)
			alert_message(client, Constants::ALERT_GUILD_EXIST)
  ...
  def change_guild_leader(client, name)
		...
		if member && @guilds[client.guild].leader != member
			...
			send_change_guild_leader(client)
		end
	end
	...
	def change_guild_notice(client, notice)
		...
		save_guild(client.guild)
		send_change_guild_notice(client)
	end

  def remove_guild_member(client, member)
		player = find_player(member)
    if player
      player.leave_guild
    else
      @guilds[client.guild].members.delete(member)
      save_guild(client.guild)
    end
    send_remove_guild_member(client, member)
  end
