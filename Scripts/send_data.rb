...
module Send_Data
	...
	def send_player_data(client, map_id)
		...
		buffer.write_byte(client.character_index)
		client.equips.each { |equip| buffer.write_short(equip) }
		buffer.write_int(client.param_base[Constants::PARAM_MAXHP])
		buffer.write_int(client.hp)
	...
	def map_message(map_id, message, player_id, color_id = Constants::CHAT_MAP)
		buffer = Buffer_Writer.new
	...
	def global_message(message, color_id = Constants::CHAT_GLOBAL)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_CHAT_MSG)
		buffer.write_byte(color_id)
		buffer.write_string(message)
	...
	def guild_message(client, message, color_id = Constants::CHAT_GUILD)
	...
	def send_animation(character, animation_id, character_type)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_ANIMATION)
		buffer.write_short(animation_id)
	...
	def send_open_create_guild(client)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_OPEN_CREATE_GUILD)
		client.send_data(buffer.to_s)
	end

	def send_open_guild(client, online_members_size)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_OPEN_GUILD)
	...
	def send_change_guild_leader(client)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_GUILD_LEADER)
		buffer.write_string(@guilds[client.guild].leader)
		client.send_data(buffer.to_s)
	end

	def send_change_guild_notice(client)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_GUILD_NOTICE)
		buffer.write_string(@guilds[client.guild].notice)
		client.send_data(buffer.to_s)
	end

	def send_update_guild(client)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_UPDATE_GUILD)
		buffer.write_string(client.guild)
		buffer.write_short(client.id)
		send_data_to_map(client.map_id, buffer.to_s)
	end
	...
	def send_remove_guild_member(client, name)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_REMOVE_GUILD_MEMBER)
	...
	def send_close_window(client)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_CLOSE_WINDOW)
		client.send_data(buffer.to_s)
	end
	...
	def send_vip_days(client)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_VIP_DAYS)
		buffer.write_time(client.vip_time)
		client.send_data(buffer.to_s)
	end
	...
	def send_logout(client)
		buffer = Buffer_Writer.new
		buffer.write_byte(Constants::PACKET_LOGOUT)
